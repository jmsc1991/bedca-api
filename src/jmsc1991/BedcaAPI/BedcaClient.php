...
class BedcaClient
{
     ...
     * @return mixed
     */
    public function getSearchFoods($search, $rawXML = false)
    {
        $searchXML = BedcaXMLRequests::getSearchFoodsXML($search);

        if ($rawXML) {
            return $this->curlXMLRequest($this->apiBase, $searchXML);
        } else {
            return $this->XMLToStdClass($this->curlXMLRequest($this->apiBase, $searchXML));
        }
    }
